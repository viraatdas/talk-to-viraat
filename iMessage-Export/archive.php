...
if(!file_exists($dirname)) {
  ...
  if (empty($at['filename']) || !file_exists($at['filename'])) {
    continue; // skip if no file or missing path
  }

  $imgsrc = attachment_folder($line['contact'], $line['date']) . $at['transfer_name'];
  if (!file_exists(dirname($imgsrc))) {
    mkdir(dirname($imgsrc), 0777, true);
  }

  // Expand tilde if needed
  $sourcePath = str_replace('~/', $_SERVER['HOME'] . '/', $at['filename']);
  copy($sourcePath, $imgsrc);
}
